...
if __name__ == '__main__':
    ...
    parser.add_argument('-c','--controller', default='XZ', help='Select the agent\'s controller')
    ...
    parser.add_argument('-r','--recordto', default='', help='npz record file')
    ...
    if args.env_id=='RoundBot-v0':
        ...
        if not args.controller in env.compatible_controllers:
            raise(Exception('Error: unknown or uncompatible controller \'' + str(args.controller) + '\' for environnement '+args.env_id))
        ...
        if args.controller=='XZ':
            env.unwrapped.load( world='rb1',
                                winsize=winsize,
                                controller={'name':'XZ','speed':args.speed,'xzrange':2},
            ...
            env.unwrapped.load( world='rb1',
                                winsize=winsize,
                                controller={'name':'Theta','speed':args.speed,'dtheta':args.dtheta},
                                global_pov=global_pov,
                                visible=args.visible,
                                perspective=not args.orthogonal,
